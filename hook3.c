...
void hook(int procID)
{
    ...
    BOOL hook = ReadProcessMemory(
        hProcess,
        (LPCVOID)0x0060FE78,  //Address we are trying to hook. Change to address of varInt
        &read,               //Buffer we are writing to
        8,        //Size of things we are reading
        NULL
    );
    printf("%p\n",read);
    int read2;
    BOOL hook2 = ReadProcessMemory(
        hProcess,
        read,  //Address we are trying to hook. Change to address of varInt
        &read2,               //Buffer we are writing to
